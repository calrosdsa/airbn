  ...
  useEffect(() => {
    ...
    return (
      <>
          ...
          ${scrolled&& 'w-full transform transition-all  duration-500  py-3  bg-white'} ${visible&& 'bg-white  transition-all transform h-screen w-screen top-0 duration-700  left-0 translate-x-0 translate-y-0'} fixed md:hidden   text-white z-10`}>
             <div className={` items-center hidden sm:flex ${scrolled&& 'hidden opacity-0'}` }  onClick={() => router.push("/")}>
          <svg
            viewBox="0 0 256 276"
            xmlns="http://www.w3.org/2000/svg"
            preserveAspectRatio="xMidYMid"
            className={`text-gray-100  font-extrabold h-[60px]  z-10 p-2 -ml-10 ${scrolled&& ' text-red-500'}`}
          >
            <path
          ...
          <span className={`text-gray-100 hidden text-xl font-extrabold ${scrolled&& 'text-red-500 '}`}>airbnb</span>
        </div >
           {visible?
           ...
           <Search className={`text-red-500 w-7 h-7 absolute top-3 sm:left-8 left-11 p-1 ${scrolled&& 'hidden'}`}/>
            ...
            className={`text-black mx-auto pl-24 sm:pl-20 flex-grow  bg-white rounded-full h-14  focus:outline-none  w-[300px] sm:w-[500px] 
            ${visible&& 'mt-3 pl-12 '}`}
